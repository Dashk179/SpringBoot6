...
public class ExcelService implements ReportService {
    ...
    private void CreateReport(){
        ...
        var sheet = worBook.createSheet(SHEET_NAME);
        sheet.setColumnWidth(0,5000);
        sheet.setColumnWidth(1,7000);
        sheet.setColumnWidth(2,3000);

        var header = sheet.createRow(0);
        var headerStyle = worBook.createCellStyle();
        headerStyle.setFillForegroundColor(IndexedColors.VIOLET.getIndex());
        headerStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);

        var font = worBook.createFont();
        ...
        var style = worBook.createCellStyle();
        ...
        for(CustomerEntity customer: customers) {
            ...
            var fileLocaiton = path+ FILE_TYPE;
            try(var outputStream = new FileOutputStream(fileLocaiton)){
